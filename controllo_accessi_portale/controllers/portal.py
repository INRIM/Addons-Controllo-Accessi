...
class CustomPortal(http.Controller):
    ...
    def badge_release_submit(self, **kwargs):
        vals = {
            ...
            'date_end' : datetime.strptime(kwargs['date_end'][:-6], '%Y-%m-%dT%H:%M:%S.%f').strftime('%Y-%m-%d %H:%M:%S'),
            'date_start' : datetime.strptime(kwargs['date_start'][:-6], '%Y-%m-%dT%H:%M:%S.%f').strftime('%Y-%m-%d %H:%M:%S'),
        }
    ...
    def get_anagrafiche(self, **kwargs):
        ...
        for record in records:
            row ={
                ...
                'present': (record.present, dict(record._fields['present'].selection).get(record.present)),
                'datetime_event' : '',
                'last_reading_event' : '',
            }
    ...
    @http.route('/get/badge_release/ca_persona_parent', auth='user', type='json', website=True)
    ...
    @http.route('/get/badge_release/tipo_enti_azienda', auth='user', type='json', website=True)
    def badge_release_tipo_enti_azienda(self, **kwargs):
        ...
        ente_azienda_ids = request.env['ca.tipo_ente_azienda'].search([('id', 'not in',
             [
                 request.env.ref('inrim_anagrafiche.tipo_ente_azienda_sede').id,
                 request.env.ref(
    ...
    @http.route('/get/badge_release/tipo_enti_azienda_hidden', auth='user', type='json', website=True)
    ...
    @http.route('/get/badge_release/work_info_type', auth='user', type='json', website=True)
    ...
    @http.route('/get/badge_release/titolo_persona', auth='user', type='json', website=True)
    ...
    @http.route('/get/badge_release/ente_azienda', auth='user', type='json', website=True, csrf=False)
    ...
    @http.route('/get/badge_release/work_info', auth='user', type='json', website=True, csrf=False)
    ...
    @http.route('/get/badge_release/tag_filter_domain', auth='user', type='json', website=True, csrf=False)
