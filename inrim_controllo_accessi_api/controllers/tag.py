...
class InrimApiTagPersona(InrimApiController):

    @http.route('/api/tag_persona', auth="none", type='http', methods=['GET'],
                csrf=False)
    def api_get_ca_tag_persona(self, **params):
        self.check_token('ca.tag_persona', 'read')
        return self.handle_response(
            *self.model.rest_get(params), is_list=True)
